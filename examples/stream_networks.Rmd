...
This document summarizes how to use `VAST` to model population dynamics along stream networks. We focus on longfin eels in the New Zealand Waitaki River catchment as an example. 

In stream networks, connected streams are likely more related in habitat and species distribution than unconnected streams in close proximity. An ideal approach to modeling stream networks would allow for spatial correlations over time or autocorrelations as a function of distance, distinguish between process and observation error particularly important when considering imperfect detection, and overcome computational challenges to analyze large networks while estimating covariance structures. 

The latter two issues for estimating spatiotemporal densities are addressed using `VAST`. `VAST` is currently used for single and multivariate spatiotemporal index standardization in stock assessments and population models. VAST has many features that make it an accessible and reproducible tool to include multiple covariates including species, ages, sizes, life stages, and habitat. Previous applications have only used Gaussian Markov random fields to approximate spatial and spatiotemporal variation, without stream network spatial and temporal correlations. 

To model stream network spatial and temporal correlations in VAST, we added a stream network spatial model that uses the Ornstein-Uhlenbeck (OU) process. This builds off of a hierarchical model implemented by Hocking et al. (2018) to model stream networks, but includes the many additional features already included in the `VAST` R package. The OU process is a stochastic process similar to a continuous version of a discrete autoregressive model. The OU process implies a child node will be correlated with its parent node as a function of distance. Variation includes an exponential rate of decay in correlation between child and parent nodes and an asymptotic variance for two sites that are far apart. 

## Data formatting: defining the stream network and including observations

The stream network is made up of stream segments, where each segment is defined by an upstream ('child') and downstream ('parent') node with a stream distance (not Euclidean distance) between the child and parent nodes. Each stream segment is associated with a unique child node. The furthest downstream (root) node is defined as an additional stream segment, where the child node is the parent node of the upstream segment, the parent node is set to zero, and the distance between parent and child is infinity. 
...
While observations cover a representative area around the lower Waitaki catchment, there happen to not be many positive encounter observations likely due to the rarity of longfin eels. 
...
Only encounter/non-encounter data (i.e. presence/absence data) are the most commonly available data type for the Waitaki catchment (as opposed to catch in numbers or density). A small jitter in encounter observations is required to fit the VAST logistic regression model.
...
We created a separate R package, `StreamUtils` to complement `FishStatsUtils` for use in VAST with the stream network spatial model. There were a few key changes from `FishStatsUtils` required for stream networks:

1. `StreamUtils::make_extrapolation_info`
  + `Region`: must equal "Stream", includes a test that throws an error if the region is anything but "Stream".
  + `stream_info`: data frame with columns Lon, Lat, and Area_km2 as before, but also including child_i, the upstream child node associated with each observation.

2. `StreamUtils::make_spatial_info`
  + arguments `LON_intensity` and `LAT_intensity` replaced with `Lon_x` and `Lat_x`. These are simply the latitude and longitude of each knot (i.e. stream network node).
  
The above adjustments ensure that the knot assigned to each observation `Spatial_List$knot_i` is the upstream child node, not the nearest node in Euclidean distance. The nearest node in Euclidean distance could be the downstream node or a node on another stream segment entirely.

Other adjustments are included in the following plotting functions associated with plotting the stream network underlying each figure or updating other plotting features:
3. `StreamUtils::plot_network`
4. `StreamUtils::plot_encounter_diagnostic`
5. `StreamUtils::plot_quantile_diagnostic`
6. `StreamUtils::plot_residuals`
7. `StreamUtils::plot_maps`
8. `StreamUtils::plot_biomass_index`
...
We create extrapolation and spatial lists to designate the spatial area of the observations and network for VAST. Using the `StreamUtils` version of the functions instead of the `FishStatsUtils` version makes sure that the correct child node is assigned to each observation, as opposed to the "nearest neighbor" approach used by default in the 2D spatial models previously included in `VAST` and accessed via `FishStatsUtils`.
```{r lists, message=FALSE, warning=FALSE, results="hide"}
Extrapolation_List = StreamUtils::make_extrapolation_info( Region="Stream", 
  stream_info=cbind("Lat"=observations_sub$lat, 
                    "Lon"=observations_sub$long,
                    "Area_km2"=1,
                    "child_i"=observations_sub$child_i), 
...
Spatial_List = StreamUtils::make_spatial_info( n_x=n_x, 
                          ...
                          Lat_x=network_sub$lat, 
                          Lon_x=network_sub$long, 
...
Options = c("CalculateRange"=1, "Calculate_effective_area"=1)
```
...
Index = StreamUtils::plot_biomass_index( TmbData=Data, Sdreport=Opt$SD, Year_Set=Year_Set, Years2Include=Years2Include, use_biascorr=FALSE )
...
Dens_xt = StreamUtils::plot_maps(plot_set=c(3), MappingDetails=MapDetails_List[["MappingDetails"]], Report=Report, Sdreport=Opt$SD, PlotDF=MapDetails_List[["PlotDF"]], MapSizeRatio=MapDetails_List[["MapSizeRatio"]], Xlim=MapDetails_List[["Xlim"]], Ylim=MapDetails_List[["Ylim"]], Year_Set=Year_Set, Years2Include=Years2Include, Rotate=MapDetails_List[["Rotate"]], Cex=1.5, pch=19, Legend=MapDetails_List[["Legend"]], zone=MapDetails_List[["Zone"]], mar=c(0,0,2,0), oma=c(3.5,3.5,0,0), plot_legend_fig=TRUE)
...
Enc_xt = StreamUtils::plot_maps(plot_set=c(1), MappingDetails=MapDetails_List[["MappingDetails"]], Report=Report, Sdreport=Opt$SD, PlotDF=MapDetails_List[["PlotDF"]], MapSizeRatio=MapDetails_List[["MapSizeRatio"]], Xlim=MapDetails_List[["Xlim"]], Ylim=MapDetails_List[["Ylim"]], Year_Set=Year_Set, Years2Include=Years2Include, Rotate=MapDetails_List[["Rotate"]], Cex=1.5, pch=19, Legend=MapDetails_List[["Legend"]], zone=MapDetails_List[["Zone"]], mar=c(0,0,2,0), oma=c(3.5,3.5,0,0), plot_legend_fig=TRUE)
